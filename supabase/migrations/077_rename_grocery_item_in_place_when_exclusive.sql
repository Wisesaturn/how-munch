...
-- 6. create_fridge_item_with_batch에 중복 정체성 검사를 추가합니다.
-- 7. 활성·비소분 품목의 정체성 유일성을 부분 유니크 인덱스로 강제하고,
--    생성 전에 기존 중복을 확인해 어떤 행이 문제인지 드러나게 합니다.
...
/* -------------------------------------------------------------------------------------------------
 * 4. create_fridge_item_with_batch — 중복 정체성 사전 검사
 * -----------------------------------------------------------------------------------------------*/
...
-- 인덱스 생성 전에 기존 중복을 확인한다.
-- 그대로 생성하면 raw 인덱스 오류만 남아 어떤 행이 문제인지 알 수 없다.
do $$
declare
  v_duplicates text;
begin
  select string_agg(format('%s / %s / %s (%s건)', name_norm, brand_norm, unit, cnt), ', ')
    into v_duplicates
  from (
    select
      lower(btrim(name)) as name_norm,
      coalesce(nullif(lower(btrim(brand)), ''), '') as brand_norm,
      unit,
      count(*) as cnt
    from public.fridge_items
    where deleted_at is null
      and not is_subdivided
    group by household_id, lower(btrim(name)), coalesce(nullif(lower(btrim(brand)), ''), ''), unit, category_id
    having count(*) > 1
  ) d;

  if v_duplicates is not null then
    raise exception
      '동일 정체성의 활성 냉장고 품목이 남아 있어 유니크 인덱스를 만들 수 없습니다: %', v_duplicates;
  end if;
end;
$$;
