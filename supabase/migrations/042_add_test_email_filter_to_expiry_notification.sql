-- ============================================================
-- Add optional test email filter to expiry notification generator
-- ============================================================
...
begin
  ...
  prepared as (
    select
      ...
      '유통기한 임박 재료가 기다리고 있어요!'::text as title,
      case
        when total_item_count > 3 then format(
          '냉장고에서 기다리고 있어요! (%s 외 %s개)... 잊으신 건 아니겠죠? 어서 소비해 주세요',
          top_item_names,
          total_item_count - 3
        )
        else format(
          '냉장고에서 기다리고 있어요! (%s)... 잊으신 건 아니겠죠? 어서 소비해 주세요',
          top_item_names
        )
      end as description,
      jsonb_build_object(
