-- Function: public.seed_search_synonyms
-- Source: supabase/migrations/082_add_search_synonym_terms.sql
-- 역할: 가구의 검색 동의어를 기본 시드 상태로 재생성하는 내부 함수입니다.
-- 동작:
-- 1. 해당 가구의 기존 동의어 단어를 모두 삭제합니다(직접 추가한 별칭 포함).
-- 2. 기본 시드 그룹을 그룹마다 새 group_key와 함께 삽입합니다.
-- 3. 삽입된 단어 수를 반환합니다.
-- 비고: 권한 검사가 없는 내부 함수다. authenticated에 노출하지 않으며,
--       사용자 호출은 멤버십을 검사하는 generate_default_search_synonyms를 거친다.
--       가구 생성 RPC와 마이그레이션 backfill처럼 auth.uid()가 없는 경로에서 직접 사용한다.
