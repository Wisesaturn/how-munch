...
export const apiResponse = {
  ...
  BAD_REQUEST(code: string, message: string, error?: unknown): Response {
    return json({ status: 400, code, message, ...(error !== undefined && { error }) });
  },
  /** 401 Unauthorized */
  ...
  /** 409 Conflict */
  CONFLICT(code: string, message: string): Response {
    return json({ status: 409, code, message });
  },
  /** 500 Internal Server Error */
  INTERNAL_ERROR(message = '서버 오류가 발생했습니다.'): Response {
    return json({ status: 500, code: 'CMN_001', message });
  },
};
